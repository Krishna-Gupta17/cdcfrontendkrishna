...
//       // Send update to backend by email
//       await axios.put(
//         `${import.meta.env.VITE_SERVER_URL}/api/member/update`,
//         {
//           email: editForm.email, // backend finds user by this email
//           codeforcesId: editForm.codeforcesId
//         },
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
...
// <h1 className="text-4xl md:text-5xl font-extrabold bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent drop-shadow-lg tracking-wide">
//       {teamName}
//     </h1>
...
//                     <button
//                       onClick={() => handleSave(localStorage.getItem("userToken"))}
...
// Dashboard.jsx
import React, { useState, useEffect } from 'react';
import { Crown, Mail, Code, Edit3, Calendar, MapPin, Trophy, Users, FlagTriangleRight } from 'lucide-react';
import { getAuth, onAuthStateChanged } from 'firebase/auth';
import axios from 'axios';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#06051C] via-[#0A0825] to-[#0D0B2E] p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Team Name with Blue Flag */}
        <div className="flex items-center justify-center gap-3 mb-8">
          <div className="w-8 h-8 text-blue-400 drop-shadow-md" />
          <h1 className="text-4xl md:text-5xl font-extrabold bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent drop-shadow-lg tracking-wide">
            {teamName}
          </h1>
        </div>
        ...
        {/* Team Members */}
<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">          {teamMembers.map((member) => (
            <div
              key={member.id}
      className={`relative transition-all duration-300 hover:scale-105 ${
        member.isLeader
          ...
          : 'bg-gradient-to-br from-blue-600/20 to-purple-600/20 border border-blue-500/30'
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                {/* Show edit button only if logged-in user is the leader */}
                {user?._id === teamMembers.find(m => m.isLeader)?.id && (
                  <button
                    onClick={() => handleEdit(member)}
                    className="text-gray-400 hover:text-white transition-colors p-1"
                  >
